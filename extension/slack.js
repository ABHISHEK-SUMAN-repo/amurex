const plt = { platform: "slack" };
chrome.storage.local.set(plt);
console.log("Slack platform local variable has been set");
...
function checkSlackMessageForMeeting() {
    console.log("check Slack Message For Meeting called");
    const messages = document.querySelectorAll('[data-qa="message-text"]');
  
    messages.forEach((msgElement) => {
      const text = msgElement.innerText;
      const match = text.match(/\/meet\/(\d+)/);
  
      if (match) {
        const meetingId = match[1];
        console.log(`Detected meetingId: ${meetingId}`);
